...
# panorama related imports
from panorama.pano_class import CylindricalStitcher
...
class VimbaPublisher(Node):
	...
	def __init__(self):
		...
		self.publisher_panorama_ = self.create_publisher(
			Image, 'vision/cam/front/panorama', 10
		)
		self.img_left = None
		self.img_middle = None
		self.img_right = None
		self.img_panorama = None
	...
	def timer_callback(self): 
		...
		img_msg_panorama = self.bridge.cv2_to_imgmsg(
			np.uint8(self.img_panorama), encoding='passthrough'
		)	
		img_msg_right.header = Header(
					frame_id="map",
					stamp=self.get_clock().now().to_msg()
		...
		self.publisher_panorama_.publish(img_msg_panorama)
...
class FrameConsumerThread(threading.Thread):
	def __init__(self, frame_queue: queue.Queue):
		...
		# panorama related constructor elements
		self.stitcher = CylindricalStitcher(FRAME_HEIGHT, FRAME_WIDTH, FOCAL_LENGTH)
		self.frame_panorama = create_dummy_frame()
	...
	def stitch(self, frame_left, frame_middle, frame_right):
		"""
		Calls the panorama stitcher function with 
		the images in the right order and structuring.
		"""
		panorama = self.stitcher.create_panorama(
			[frame_middle, frame_left, frame_right]
		)
		return panorama
	...
	def run(self):
		...
		while alive:
			...
			if frames:
				...
				# cv2.imwrite("middle.png", frame_middle)
				# cv2.imwrite("right.png", frame_right)

				# we can take this part out and only publish the frames,
				# but for the time being, the panorama will be published here
				try:
					self.frame_panorama = self.stitch(frame_left, frame_middle, frame_right)
				except:
					pass
				
				self.publisher.img_panorama = self.frame_panorama.copy()

				# separate image publisher
