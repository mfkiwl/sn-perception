...
# frame parameters (camera)
FRAME_QUEUE_SIZE = 10
FRAME_HEIGHT = 720
FRAME_WIDTH = 1280

# image parameters (processing)
IMAGE_HEIGHT = 384
IMAGE_WIDTH = 680
FOCAL_LENGTH_INDOORS = 540
FOCAL_LENGTH_OUTDOORS = 510
...
def resize_if_required(frame: Frame) -> numpy.ndarray:
    ...
    if (frame.get_height() != IMAGE_HEIGHT) or (frame.get_width() != IMAGE_WIDTH):
        cv_frame = cv2.resize(cv_frame, (IMAGE_WIDTH, IMAGE_HEIGHT), interpolation=cv2.INTER_AREA)
